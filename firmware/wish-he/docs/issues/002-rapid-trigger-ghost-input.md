...
3. 거기서 0.10mm 를 다시 누른다 → RT 재입력이 `pressed` 를 세운다.
...
바로 아래 떼진 가지(`keys.c:2699`)는 RT 가 걸린 동안 **절대 입력지점을 일부러 안
본다.** 둘을 `or` 로 묶었다가 올라오는 내내 같은 글자가 쏟아졌던 일이 주석에 그대로
적혀 있다. 눌린 가지만 그 처리를 못 받았다.
...
유령 입력이 매번 안 나가는 것도 같이 설명된다. `pressed[]` 가 26us 동안만 서 있고
`matrix_scan()` 이 그걸 훑는 주기는 훨씬 길다 — 창이 겹칠 때만 호스트로 간다. 그래서
솔직한 연속 입력이 아니라 짧고 들쭉날쭉한 것으로 보인다.

### 왜 지금까지 안 드러났나

절대 해제는 **재입력이 해제지점보다 얕은 곳에서 일어날 때만** 끼어든다. 기본값
(1.00mm / 0.50mm)으로 쓰면 재입력이 대개 0.50mm 보다 깊어서 안 걸린다. 입력지점을
2.45mm 로 내리면 행정의 거의 전부가 해제지점 위라 늘 걸린다.
...
RT 가 꺼져 있으면 `rt_active` 가 거짓이라 **지금과 하나도 다르게 안 돌아간다.**

절대 해제를 통째로 지우면 키가 굳는다. 재입력 0.10mm · RT 해제 0.50mm 로 두고 깊이
0.15mm 에서 눌렸다면 `peak` 이 0.15mm 라, 손을 다 떼도 되돌림이 0.15mm 뿐이라
0.50mm 를 영영 못 넘는다.
...
`rt_arm` 을 푸는 자리는 하나뿐이고, 기준은 **쉬는 자리**다.
...
한 번 입력지점을 넘고 나면 손을 뗄 때까지 RT 가 전 행정에서 산다. 이슈 제목이 말하는
것이 이것이다 — 연속 RT 를 껐는데도 입력지점 위, 해제지점 위에서 키가 걸린다.
...
지금의 비연속이 흔한 규약의 **연속**이고, 지금의 연속은 거기서 한 칸 더 넓다.
...
| 수정안 A 구현 + 전후 성능 측정 | 필요 | 위 패치. 원인 B 와 무관하게 먼저 끝낼 수 있다 |
| 입력지점 1.00mm 가 몇 카운트인지 `keys rt` 로 확인 | 필요 | 원인 B 를 고칠지 판단하는 숫자. `t_rt_arm_release` 의 깊이 281 이 그보다 위인지 아래인지 |
| 눌림이 정말 한 스캔만 서 있는지 확인 | 필요 | `keys lat` 과 위 시험. 1~3ms 는 보고자 영상의 숫자이지 우리가 잰 값이 아니다 |
| 원인 B 규약 결정 | 불필요 | 결정만 나면 한 줄 + 시험 수정. **A 가 끝난 뒤에 잡는다** |
...
- 1~3ms 라는 폭은 보고자 영상에서 나온 값이다. 우리 쪽 수치는 `keys lat` 으로 따로
  재야 한다.
- 이 보드에서 입력지점 1.00mm 가 몇 카운트인가. 원인 B 의 수정 결과가 거기서 갈린다.
