...
- [ ] **WS2812 인코딩을 나눠 굽기** (지금은 안 해도 된다)

      RGB 프레임 하나가 태스크를 193us 잡는다. **전송은 이미 비동기다** —
      `ws2812Refresh()` 는 앞 프레임이 나가는 중이면 그냥 건너뛴다. 시간을 먹는 것은
      색 83개를 비트 패턴 1992바이트로 펼치는 **인코딩**이라 DMA 로 넘길 수가 없다.

      가르면 된다 — 루프마다 4분의 1씩 채우고 다 차면 DMA 를 건다. 193us 봉우리
      하나가 50us 넷이 된다.

      ★ **지금은 해가 없다.** 스캔은 ADC 가 모는 별도 경로라 28us 그대로고(초과
      0~1회), USB 폴링 1ms 에 193us 면 여유가 크다. 효과가 무거워지거나 LED 가
      늘면 그때 가른다
