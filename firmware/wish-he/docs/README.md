...
- [ ] **보정을 VIA HE 에서** — HE 탭의 `CALIBRATION` 섹션이 자리만 잡혀 있다
      (`todo: porting the CLI keys cal flow`). CLI `keys cal` 은 전 키를 끝까지
      눌러 바닥값을 모으는 대화형 흐름이라, 어느 키가 남았는지 보여주는 화면이
      필요하다 — 키보드 그림에 진행 상황을 칠하면 CLI 의 레이아웃 뷰와 같은 일을
      더 잘 할 수 있다. 펌웨어 쪽 통로는 아직 없다 (HID 명령을 새로 뚫어야 한다)
