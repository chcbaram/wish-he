...
- [ ] **프로파일 4개** — 설정 한 벌을 네 벌로 늘리고 키로 전환. **펌웨어 일이다** —
      `cfg` 한 벌만 두고 있으므로 저장 구조부터 바꿔야 한다. 내보내기·가져오기는
      끝났다 (VIA HE 의 장치 > BACKUP). 보정값은 일부러 안 담는다 — 보드마다 다른
      값이라 옮기면 mm 가 눈에 안 보이게 틀어진다
