...
def t_rt_arm_release():
    ...
    ★ **깊이를 RT 구간 안으로 옮겼다** (#2-B 를 고치면서, 2026-09-18).

      RT 가 사는 구간의 위 경계가 이제 입력지점이라(`t->rt_arm_lo`), 예전처럼
      0.68mm — 입력지점 1.00mm 보다 얕은 자리 — 에서 재입력을 기대할 수 없다.
      거기서는 규약상 RT 가 죽어 있는 것이 맞다.

      ★ 그래서 **옛 버그 자체는 이 시험이 더는 못 잡는다.** 옛 기준(해제지점
        0.80mm)은 새 기준(입력지점 0.95mm)보다 늘 얕아서, 새 규약을 통과하는 손
        놀림은 옛 규칙도 자동으로 통과한다. 구조적으로 닿을 수 없게 됐다 —
        시험이 약해진 것이 아니라 그 자리가 없어졌다.

      남은 것은 여전히 값지다 — **해제지점을 올려도 RT 구간 안에서는 RT 가 산다.**
      해제지점이 RT 를 갉아먹지 않는다는 보장이 그대로다.
    """
    try:
        ...
        ★ 되눌림을 **넉넉히** 준다. 형제 시험(`RT 가 스트로크 안에서 반복해 산다`)에
          ...
          같은 mm 가 구역에 따라 카운트로 다르게 나온다. 0.40mm 면 0.10mm 문턱에
          여유가 넉넉하다.
        """
        p, d = step(160)
        if not p:
...
@test("rt", "연속 RT 에서 얕은 자리의 재입력이 유지된다 (#2-A)")
def t_rt_cont_shallow_hold():
    """
    ★ 절대 해제가 RT 가 세운 눌림을 다음 스캔에 도로 지우던 것.

      입력 2.45mm / 해제 2.40mm 처럼 행정의 거의 전부가 해제지점 위인 설정에서,
      RT 재입력이 해제지점보다 얕은 곳에서 걸리면 바로 다음 스캔에 절대 해제가
      뗐다. 눌림이 한 바퀴(26us)만 서 있다 떨어져 호스트에는 1~3ms 짜리 유령
      입력으로 보인다 (이슈 #2 원인 A).

    ★ **연속 RT 로 잰다.** 비연속에서는 #2-B 를 고친 뒤로 이 자리에 닿을 수가
      없다 — RT 가 입력지점 위에서 아예 안 살기 때문이다. 연속 RT 는 전 행정에서
      RT 가 사는 것이 그 기능이라 거기서만 두 규칙이 마주친다.

    ★ 주입은 깊이를 가만히 붙잡아 주므로 스캔 단위 타이밍을 맞출 필요가 없다.
      안 고친 펌웨어에서는 **검사가 들여다볼 때 이미 떼져 있다.**
    """
    try:
        ...
        p, d = step(70)                    # 재입력 거리 이상 다시 누른다
        if not p:
            return f"재입력이 유지되지 않는다 (깊이 {d}) — 절대 해제가 RT 를 덮었다"
        return None
    finally:
        say("keys rt cont off")            # 설정을 먼저, 주입은 rt_restore 가 끈다
        rt_restore()


@test("rt", "비연속 RT 는 입력지점 위에서 안 걸린다 (#2-B)")
def t_rt_arm_window():
    """
    ★ 연속 RT 를 껐는데 켠 것처럼 굴던 것 (이슈 #2 원인 B).

      예전에는 `rt_arm` 을 **쉬는 자리**에서만 풀었다. 그래서 한 번 입력지점을
      넘고 나면 손을 완전히 뗄 때까지 전 행정에서 RT 가 살아 있었다 — 그게 바로
      연속 RT 의 동작이다. 연속 RT 를 끈 사람이 얕은 자리에서 재입력을 맞았다.

      RT 가 사는 구간의 위 경계는 **입력지점**이다. 위로 올라오면 RT 는 죽고,
      입력지점을 다시 넘어야 살아난다.
    """
    try:
        rt_setup(100, 50, 10, 20)          # 입력지점 1.00mm, 연속 RT 는 끈 상태
        if not step(200)[0]:
            return "깊게 눌렀는데 안 잡힌다"
        if step(30)[0]:
            return "0.30mm 까지 올렸는데 안 떼졌다"

        p, d = step(70)                    # 입력지점 위에서 0.40mm 되누른다
        if p:
            return f"입력지점 위(깊이 {d})에서 RT 가 걸렸다 — 비연속인데 연속처럼 산다"

        p, d = step(150)                   # 입력지점을 다시 넘으면 살아야 한다
        if not p:
            return f"입력지점을 넘었는데 안 눌린다 (깊이 {d})"
        return None
    finally:
        rt_restore()
