...
void keysProfChanged_kb(uint8_t idx)
{
  ...
  {
    /*
     * ★ 바꾸기 **전에** 눌린 키를 비운다. 안 그러면 그 키가 호스트에 영원히 남는다.
     *
     *   QMK 는 뗄 때 키코드를 (레이어, 행, 열)로 **다시 읽는다.** 누를 때 캐시해
     *   두는 것은 레이어 번호뿐이다(quantum.c 의 layer cache). 그런데 프로파일마다
     *   키맵이 따로라(dynamic_keymap 이 keymapProfileOffset 을 더한다) 같은 자리가
     *   새 프로파일에서는 **다른 키코드**다.
     *
     *   실제로 재현했다 — 프로파일 1 에서 X 를 누른 채 2 로 옮기고 손을 떼니,
     *   해제는 프로파일 2 의 Z 로 나가고 **X 는 아무도 안 떼서 호스트에 남았다.**
     *   같은 키를 다시 눌렀다 떼기 전에는 안 풀린다.
     *
     *   여기는 아직 옛 프로파일이라 해제가 제대로 나간다. 바꾼 뒤에 비우면 이미
     *   늦다 — 그때는 키코드가 벌써 새 것이다.
     *
     *   덤으로 SOCD·연타의 눌림 기억(userLoad 가 지운다)과도 앞뒤가 맞는다.
     *   한쪽만 지우면 "물리적으로 누르고 있는데 호스트에는 없는" 상태가 된다.
     */
    clear_keyboard();
