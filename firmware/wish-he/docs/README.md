...
| [15. 거리를 제대로 재기](15-distance-curve.md) | 데이터시트 **두 점**으로 곡선을 계산한다 — 실측이 필요 없다. 심으로 **직선이 틀린 양까지** 검증. **참고 보드는 끝까지 직선이다** (디스어셈블·JS 로 확인). 역수 표로 나눗셈 걷어내기, RT 를 깊이 구역마다, 데드존 ↔ 표시 스퀄치 | 완료 |
