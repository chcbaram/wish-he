...
- [ ] **래피드 트리거를 깊이에 따라 재단** — 곡선을 넣고 남은 하나다.

      절대 위치(입력·해제·데드존·바닥)는 곡선으로 정확해졌는데, RT 의 두 값은
      **위치가 아니라 이동량**이라 아직 직선이다. 곡선에서는 같은 이동량이라도
      어디서 움직였느냐에 따라 카운트가 달라서, 설정한 숫자가 한 지점에서만 맞는다.

      실측이 아니라 곡선에서 바로 나오는 값이다 (RT 0.50mm 설정 기준) —

      ```
      지금 눌린 깊이     실제 되돌림 거리
         1.00 mm            1.00 mm    너무 둔하다
         2.00 mm            0.60 mm
         3.00 mm            0.30 mm    설정의 60%
         3.30 mm            0.24 mm    설정의 절반
      ```

      **깊이 눌린 상태일수록 두 배로 예민해진다.** 타이핑에서 손가락이 머무는 곳이
      거기라, RT 를 켜면 설정한 숫자보다 훨씬 떤다.

      고치는 법 — 깊이를 8구역쯤으로 나눠 구역마다 RT 카운트를 `keysThrRebuild` 에서
      미리 계산해 둔다. 뜨거운 루프에서는 배열 색인 한 번이면 되고, 나눗셈도 역변환도
      없다. 키당 8바이트씩 512바이트다.

      ★ **이걸 하고 나서 RT 를 만져야 한다.** 지금 값으로 튜닝해 봐야 깊이마다 다르게
      동작해서 감이 안 잡힌다
...
- [ ] **RT 실사용 튜닝** — 재입력·입력 해제 거리의 경계 찾기.
      ...
      **곡선 전환 뒤에 하는 게 낫다** — 지금 값은 0.8mm 어긋나 있다
