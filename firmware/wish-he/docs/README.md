...
- [ ] **굽다 만 앱이 그대로 실행된다** — IAP 는 `0x20004` 로 그냥 뛴다.

      참고 보드의 최신 펌웨어는 앱 앞에 작은 stub 을 두고 **CRC-32 로 이미지를 검사**한
      뒤에야 본체로 뛴다. 틀리면 스스로 부트 플래그(`0x1D000`)를 세우고 재시작해
      업데이트 모드로 되돌아간다. 부트로더는 못 바꾸지만 **stub 은 우리 앱 앞에 넣을 수
      있다.** 구조는 `docs/analysis-v173/README.md` §2 에 다 적어 뒀다.

      ★ 급하지는 않다 — 지금도 부트로더가 자기 자신을 못 덮어쓰므로 다시 구우면 된다.
