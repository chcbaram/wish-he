...
- [ ] **보정을 VIA HE 에서** — HE 탭의 `CALIBRATION` 섹션이 자리만 잡혀 있다
      ...
      **보정이 두 가지라는 것부터 화면에 드러나야 한다.**
      ...
      3. HE 탭에서 키보드 그림에 진행 상황을 칠한다. CLI 의 격자 뷰보다 낫고
         어느 키가 남았는지 한눈에 보인다
- [ ] **보정 화면에 지금 보정값 표시** — 보정 메뉴에서 **저장돼 있는** 보정을
      읽어 키보드 그림에 표시한다. 지금은 보정을 도는 동안에만 칠해지고, 평소에는
      어느 키가 이미 보정됐는지 볼 길이 없다. 키별 스트로크도 같이 보이면 편차
      (실측 13%)가 눈에 들어온다. 통로는 열려 있다 — `keysGetKeyCfg`(0xC5)가
      키별 보정 플래그와 값을 준다
- [ ] **키캡에 키별 값 표시** — 스트로크·보정 여부·키별 입력지점을 키캡 위에.
      `keycap.tsx` 는 VIA 상류 파일이지만 포크를 따로 관리하므로 고쳐도 된다
