...
- [ ] **태스크 최대 1685us 가 어디서 나는지** — 디버그 화면에서 드러났다.
      ...
      **셋 다 1685us 가 아니다.** 부팅 직후나 USB 열거 때 한 번 나는 값일 가능성이
      ...
      ★ **RGB 의 200us 봉우리도 같은 식구로 보인다.** 상시 작업으로는 31us 인데
        최대만 200us 다. 실제로 굽고 4초 뒤에는 `task_over 421`, 20초 두고 다시
        재니 `8` 이었다. 크기를 깎을 것이 아니라 **튀는 조건**을 찾아야 한다 —
        플래시 쓰기(XIP, 인터럽트 6ms 차단), USB 열거·서스펜드, 부팅 직후가 후보다
