 ...
 *
 *  ★ 부동소수를 안 쓴다. FPU 가 없어서가 아니다 — 칩에는 있다.
 *
 *    HPM5300 계열은 배정밀도 부동소수를 지원한다고 제조사가 밝히고 있다. 없는 것은
 *    빌드 쪽이다. CMakeLists 가 -march=rv32imac / -mabi=ilp32 로 잡아 소프트 float 다.
 *
 *    하드 float 로 바꾸려면 -mabi=ilp32f 가 되는데 ABI 가 갈려 SDK 와 newlib 을 통째로
 *    다시 구워야 한다. 그런데 이 계산은 부팅 때 한 번 돌고 스캔 경로는 어차피 정수여야
 *    하므로 얻을 것이 없다. 정수로 두는 것은 그 선택이지 제약이 아니다.
 *
...
static void keysThrRebuild(void)
{
  ...
  for (uint32_t i = 0; i < KEYS_MAX; i++)
  {
     ...
     *     0.50mm -> 144         3.6x
     *
     * ★ "전 행정의 1/10 밑으로 못 내리게" 같은 **비율 규칙은 안 쓴다.**
     *
     *   눈금이 고운 쪽일수록 손해다. 254칸 눈금이면 1/10 이 25칸이지만 우리는
     *   2514칸이라 251카운트(0.80mm)가 되어 쓸 만한 구간을 통째로 막는다.
     *   하한은 눈금이 아니라 **잡음**에서 나와야 한다.
     *
...
ATTR_RAMFUNC static void keysTrack(uint32_t step)
{
  ...
  for (uint32_t c = 0; c < KEYS_CH_MAX; c++)
  {
     ...
     *
     * ★ 눌린 것으로 판정된 셀은 건드리지 않는다.
     *
     *   원래는 "눌려 있는 셀은 d 가 밴드를 넘어서 영향받지 않는다" 로 두었다.
     *   **그 전제가 곡선을 넣으면서 깨졌다.** 지금 입력지점 0.50mm 는 145 카운트라
     *   밴드(150) 안이다. 그러면 키를 얕게 문 채로 있어도 기준값이 따라 내려와
     *   저절로 떼진다 — `keys inject` 로 재니 입력지점 0.30mm 에서 깊이가
     *   116 -> 14 로 줄며 **약 18초 만에 해제됐다.** 손가락은 그대로였다.
     *   게임용 얕은 설정이 정확히 그 구간이다.
     *
     *   깊이(d)로 거르는 대신 **판정 결과로 거른다.** 기준값은 "안 눌린 상태의
     *   값" 이므로 눌려 있는 동안 움직일 이유가 애초에 없다. 이렇게 두면 사용자가
     *   문턱을 어디에 두든 안전하다 — 상수끼리 크기를 맞춰 둘 필요가 없어진다.
     *
     *   판정은 이번 반복 뒤에 갱신되므로 여기서 보는 것은 **직전 스캔의 결과**다.
     *   26us 지연이라 뜻이 없다.
     */
    ...
    else
    {
      ...
      /*
       * RT 를 언제 푸는가 — **키가 원위치로 돌아왔을 때**다.
       *
       * 연속 RT 면 풀지 않는다 — 전 구간에서 살아 있는 게 그 기능이다.
       *
       * ★ 예전에는 `d < t->release` 로 판정했다. 주석은 "입력지점 아래로 완전히
       *   돌아왔으면" 이라고 적혀 있었는데 코드는 **해제지점**을 보고 있었다.
       *
       *   그러면 RT 해제가 난 자리가 해제지점보다 얕을 때 **같은 스캔에 RT 가
       *   풀린다.** 그 뒤로는 절대 입력지점까지 눌러야만 다시 살아난다 — 사용자
       *   눈에는 "가끔 리셋이 안 먹는다" 로만 보인다.
       *
       *   재현했다. 입력지점 1.00mm, RT 입력해제 0.20mm 로 두고 깊이 281 에서
       *   재입력을 시켰더니 —
       *
...
static void keysCfgDefault(void)
{
  ...
  for (uint32_t pi = 0; pi < KEYS_PROF_CNT; pi++)
  {
     ...
     *
     *   화면 떨림은 KEYS_SQUELCH_UM 이 따로 맡는다. 데드존만은 하한
