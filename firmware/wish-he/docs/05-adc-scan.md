...
|  | 상용 | 우리 |
|---|---|---|
| 생 스캔 | **25.0 us** (40 kHz) | 62 us (QMK 전 38 us) |
| 스텝당 | 3.12 us | 7.75 us |
| 누적 | **3개** (√3 노이즈 감소) | 없음 (데드밴드 필터) |
| 유효 키 레이트 | 13.4 kHz | 16 kHz |
...
우리 스텝이 두 배 넘게 걸리는 이유는 분명하다. **상용은 스텝 안에서 생값을 누적만
하고**, 우리는 `keysFilter` 8번(데드밴드 분기 포함)에 `keysTrack` 까지 스텝 안에서 한다.

```
12편  스텝 루프에서 필터·추적을 빼서 생 스캔을 25us 대로 내린다
      (그리고 스캔을 슈퍼루프에서 떼어낸다 — 10편에서 38 -> 62us 로 늘었다)
