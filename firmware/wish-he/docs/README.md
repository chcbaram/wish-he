...
`spi_transfer()` 폴링 전송이라 프레임당 **약 2ms 동안 CPU 가 묶인다.**
키 스캔이 들어오면 상용 펌웨어처럼 **SPI + DMA** 로 바꿔야 한다
(상용은 `hpm_spi_dma_install_callback` 을 쓴다 — 그래서 40kHz 스캔과 공존한다).

`m483-fw` 의 ws2812.c 는 한 걸음 더 나가 **PDMA 순환 scatter-gather** 로 무정지
전송을 한다(재시작 글리치 제거). HPM5361 로 옮길 때 참고할 만하다.
...
- [ ] WS2812 를 SPI+DMA 로 전환 (현재 블로킹 2ms)
