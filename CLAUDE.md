...
- **구현 자체를 설명한다.** 다른 제품·보드를 견주어 쓰지 않고, 제품·회사 이름도 쓰지
  ...
  ★ **딱 하나 예외 — 루트 `README.md` 의 "지원 PCB" 표.** 우리는 PCB 를 만들지 않고
    상용 보드에 올린다. 어느 보드를 사면 되는지를 안 적으면 **쓸 수가 없다.**
    그래서 거기에는 제조사·제품명을 적는다 (Geonworks VENOM60HE 7U ·
    EverGlide AE61 Pro).

    범위는 그 표뿐이다. 편(`docs/`)과 코드 주석은 규칙 그대로다 — 거기서 제품
    이름이 필요해 보이면 대개 **구현 대신 견줌을 적고 있는 것**이다.
