...
| 10. QMK 얹기 | `quantum` 을 모듈로 이식, `port/matrix.c` 에 `keysGetRow()` 물리기, EEPROM 백엔드 — **레이어·매크로·키오버라이드를 직접 안 만들려면 이게 먼저다** | 예정 |
| [11. VIA 붙이기](11-via.md) | `quantum/via.c` + `dynamic_keymap`, raw HID `0xFF60`. **라이브 트래킹은 장치가 민다**, 레이아웃도 장치가 준다 (JSON 없이) | 진행 |
| 12. 8kHz 로 올리기 | 완료 콜백에서 재무장 — 스캔은 이미 예산의 30% | 예정 |
| 13. 래피드 트리거 | ADC 값 → 거리, 액추에이션 조정. 튜닝 화면은 11편의 VIA 포크에 얹는다 | 예정 |
