...
- [ ] **태스크 최대 1685us 가 어디서 나는지** — 디버그 화면에서 드러났다.
      ...
      ```
      task_avg     3 us            평균은 짧다
      task_max  1685 us            그런데 최악이 1.7ms
      task_over 3251 / 256만       125us 초과 0.13% (800번에 한 번)
      ```

      넘긴 회차는 리포트가 한 프레임 늦는다. 짚이는 자리는 설정 저장(플래시 6ms)과
      조명 프레임(`rgb_max` 439us)인데 **둘 다 1685us 가 아니다.** Clear 를 누르고
      RGB 를 끈 채 두면 갈린다 — 짐작 말고 그렇게 가를 것
