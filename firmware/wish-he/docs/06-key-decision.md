...
그래서 스캔 처리의 첫 줄에서 `>> 4` 로 내리고, 이후 판정·저장·표시가 전부 12비트를
쓴다. 12비트로 내려도 실측 노이즈가 ±6 이라 판정에 쓸 여유가 충분하다.
...
그래서 데드밴드로 바꿨다. 밴드보다 큰 변화는 **같은 샘플에서 즉시** 반영되므로
...
키가 눌린 순간부터 호스트가 그걸 받기까지 어디서 얼마를 쓰는지 세어 둔다.
8kHz 폴링(125us)을 전제로 한다.

| 단계 | 시간 |
|---|---|
| ADC (12비트, clk_div 4, sample_cycle 5) | 스캔에 포함 |
| 스캔 한 바퀴 | 38us (실측) |
| 필터 | **0** — 데드밴드라 지연이 안 붙는다 |
| 리포트 대기 (8kHz 폴링) | 0 ~ 125us |
| **합** | **40 ~ 165us** |

★ 필터가 0 인 것이 이 표의 핵심이다. IIR 을 썼다면 여기에 152~342us 가 그대로
얹혔다. 7편에서 리포트 경로를 완료 콜백에서 즉시 재무장하도록 만들어 뒤쪽도 뗐다.
