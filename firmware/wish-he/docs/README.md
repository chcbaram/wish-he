...
- [ ] **`rgb_matrix` 얹기** — 막고 있던 것이 다 풀렸다. 전류 모델·상한(450mA)·
      밝기 상한(18)·LED 매핑(`layout.h` 의 `keys_led[]`)이 준비돼 있다.
      얹은 뒤 `ws2812 info` 의 `limit hit` 가 0 인지 확인한다 — 0 이 아니면 밝기
      상한이 예산보다 높아 켜진 개수에 따라 밝기가 출렁인다 ([14편](14-led-limiter.md))
      - 걸림돌 둘: 스페이스바는 한 스위치에 LED 가 셋, 스플릿 백스페이스 자리는
        스위치가 없는데 LED 는 있다. `g_led_config` 를 만들 때 그대로 문제가 된다
