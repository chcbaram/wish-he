...
- [ ] **★ 굽고 나면 펌웨어 화면이 굳는다** (오늘 넣은 버그) — 펌웨어는 정상으로 올라가고
      "Done" 까지 뜨는데 버튼이 다 죽는다. 브라우저를 새로고침하면 정상이다.

      원인은 `iapFind()` 다. `navigator.hid.getDevices()` 는 **권한을 준 장치를 뽑혀
      있어도 계속 돌려준다.** 그래서 한 번 구운 뒤에는 부트로더가 사라져도
      `bootloaderSeen` 이 참으로 남는다.

      `he.tsx` 의 `bootOnly = bootloaderSeen && !api` 이므로, 굽고 나서 앱이 다시
      열거되기 전 `api` 가 잠깐 null 인 사이에 **부트로더 화면으로 굳는다** — 그 화면은
      갈래를 `device/firmware` 하나로 잠그고 있어서 버튼이 다 죽은 것처럼 보인다.

      고칠 방향 둘 —

        - `bootOnly` 를 **굽는 중(`busy`)에는 아예 판단하지 않게** 하고, `api` 가
          살아나면 즉시 푼다 (가볍다, 이쪽을 권한다)
        - `iapFind()` 가 `open()` 을 시도해 실제 연결 여부까지 보게 한다 (폴링마다
          열고 닫는 부담이 있다)

      via-he 의 `components/panes/he.tsx`, `components/Home.tsx` 두 자리다
