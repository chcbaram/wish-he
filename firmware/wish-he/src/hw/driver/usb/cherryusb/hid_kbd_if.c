 ...
 *   있으면 NAK 이고 그 폴링은 그냥 날아간다.
 *
 *   그런데 CherryUSB 의 usbd_ep_start_write() 는 one-shot 이다 — 큐가 없어서 한 번에
 *   하나만 실린다. 그래서 8kHz 를 유지하려면 완료 콜백에서 곧바로 다음 것을 실어야
 *   한다. 스캔이 끝나기를 기다렸다가 실으면 그 사이의 폴링을 놓친다.
 *
 *   상용 보드도 정확히 이 구조다. 완료 콜백에서 in-flight 플래그를 지우고 즉시
 *   송신 루틴으로 되돌아간다.
 *
 *   그래서 여기서는 스캔과 전송을 갈라놓는다.
 *     스캔  -> hidKbdSetReport() 로 섀도 버퍼만 갱신   (주기 무관)
 *     완료  -> 섀도를 USB 버퍼로 복사하고 즉시 재무장  (125us 마다)
 */
...
/* 완료 콜백(ISR) — 곧바로 다음 리포트를 싣는다. 이게 8kHz 를 유지하는 지점이다. */
...
void hidKbdSetReport(uint8_t modifier, const uint8_t *keys, uint32_t cnt)
{
  ...
  {
    shadow[2 + i] = (i < cnt) ? keys[i] : 0;
  }

  /*
   * 전송이 놀고 있으면 여기서 깨운다.
   *
   * 보통은 완료 콜백이 이어받지만, 열거 직후나 호스트가 잠깐 쉰 뒤에는 체인이
   * 끊겨 있다. 그때 다시 물리는 자리다.
   */
