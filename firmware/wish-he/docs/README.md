...
- [ ] **거리 환산을 곡선(LUT)으로** — 여기까지 왔다.

      끝난 것 — 근거 자료([ref/he-magnet-model.md](ref/he-magnet-model.md)),
      생성기([../tools/he_magnet_fit.py](../tools/he_magnet_fit.py)), 스위치 표에
      데이터시트 두 점, VIA HE 의 **스위치 갈래**에 ADC↔거리 그림과 실시간 자리.

      **`GEON RAW HE` 실제 제원은 160 → 720 Gs, 3.4mm** (제조사 공개, 검색으로 확인).
      그걸로 계산한 지금의 오차 —

      ```
      입력지점 0.50 mm 로 잡으면 실제로는 1.10 mm 에서 눌린다  (+0.60)
      입력지점 1.00 mm 로 잡으면 실제로는 1.79 mm 에서 눌린다  (+0.79)
      최대 오차 0.80 mm @ 실제 2.1 mm
      ```

      남은 일 —

      1. (선택) 심 한 장으로 **한 점만** 검증. 1.0mm 심을 끼우고 끝까지 눌렀을 때
         화면의 모델 값이 1.0mm 라고 하는지. 전수 측정은 불필요
      2. `keys_switch[]` 에 LUT 33칸을 상수로 (생성기가 뽑아 준다).
         데이터시트에서 계산되는 값이라 EEPROM 에 둘 이유가 없다
      3. `keysGetDepthUm()` 을 이진 탐색 + 선형 보간으로. 33칸이면 비교 5회라 싸다
      4. 바꾼 뒤 스위치 화면에서 두 선이 겹치는지 확인 — 그게 곧 검증이다

      ★ **화면이 펌웨어보다 먼저**인 이유를 지켰다. 펌웨어를 먼저 바꾸면 모든 키의
      감각이 한꺼번에 달라지는데 그게 맞는지 볼 방법이 없다. 지금은 두 값을 나란히
      놓고 볼 수 있다
