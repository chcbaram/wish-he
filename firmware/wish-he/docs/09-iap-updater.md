...
브라우저(WebHID)에서는 이것이 **권한 문제로 번진다.** 권한은 장치 단위로 주는데
부트로더는 다른 장치로 잡히므로, 앱을 허용해 두어도 부트로더는 따로 허용받아야 한다.
그런데 앱 모드일 때는 부트로더 장치가 존재하지 않아 **미리 받아둘 수도 없다.**
부트로더가 뜬 뒤에 한 번 받는 수밖에 없고, 그 뒤로는 브라우저가 기억한다
(오리진 + 프로필 단위).
