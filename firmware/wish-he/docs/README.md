...
- [ ] **키캡에 키별 값 표시** — 스트로크·보정 여부·키별 입력지점을 키캡 **위에**.
      지금은 키가 칠해지느냐 마느냐(두 값)뿐이라 "보정됨" 은 알아도 "얼마로
      보정됐나" 는 못 본다. 편차 13% 가 보정을 하는 이유 자체인데 그 숫자가
      화면에 없다. `keycap.tsx` 는 VIA 상류 파일이지만 포크를 따로 관리하므로
      고쳐도 된다

      먼저 정할 것 — 칠하기(overlayKeys)는 두 값이고 숫자는 값이 연속이다.
      같은 통로로 못 보낸다. 그림에 값을 보내는 두 번째 통로가 필요하다
