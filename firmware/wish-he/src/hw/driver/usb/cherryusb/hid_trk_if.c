 ...
 *   어긋나 표준 VIA 도구와 공존이 안 된다. 상용 보드도 IN 전용 인터페이스를 따로
 *   두고 있다.
 *
 * ★ 왜 요청-응답이 아니라 밀어내나.
 *
 *   상용은 페이지 번호를 받아 16키씩 돌려주는 요청-응답이라 스냅샷 하나에 왕복이
 *   여러 번이다. 켜두고 계속 내보내면 왕복이 없어 폴링 주기가 곧 프레임 주기다.
 *
 ...
 * 실으면 호스트가 임계값을 몰라도 상용 화면처럼 체크 표시를 그릴 수 있다.
 *
