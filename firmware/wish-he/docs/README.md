...
- [ ] **보정을 VIA HE 에서** — HE 탭의 `CALIBRATION` 섹션이 자리만 잡혀 있다
      ...
      **보정이 두 가지라는 것부터 화면에 드러나야 한다.**
      ...
      3. HE 탭에서 키보드 그림에 진행 상황을 칠한다. CLI 의 격자 뷰보다 낫고
         어느 키가 남았는지 한눈에 보인다
- [ ] **입력지점도 키캡에 찍기** — 통로는 이미 열렸다. 설정 화면에서 키별
      입력지점·해제지점을 키캡 우하단에 올리면 "고른 키만 다르게 해 뒀다" 가
      한눈에 보인다. `overlayText` 에 넣기만 하면 된다
...
- **폴링 한 회차가 여러 왕복이면 `setInterval` 을 쓰지 않는다** — 주기 안에 못
  끝내면 회차가 겹쳐 같은 통로에서 응답이 엇갈린다. 끝난 뒤에 다시 건다
