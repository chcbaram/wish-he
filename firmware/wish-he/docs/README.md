...
- [ ] **`hold_okp` 와 0번 프로파일 조명칸이 겹친다** (버그) — 둘 다
      `EECONFIG_USER_DATABLOCK + 0` 이다. `profRgbStore(0)` 이 `hold_okp` 를 덮어쓰고,
      *Hold On Other Key Press* 를 토글하면 0번 프로파일의 조명 켬/모드가 깨진다.
      `_Static_assert` 는 총량만 봐서 못 잡는다. 조명칸을 `+8` 부터 잡으면 끝난다
      ([11편](11-via.md))
