...
- [ ] **잡음이 셀마다 독립인가, 공통인가** — 지금 가장 값진 측정이다.

      잡음 바닥이 우리 하한 셋을 전부 정한다 (`KEYS_RT_MIN_CNT 60` = 입력지점 하한
      0.22mm, `KEYS_SQUELCH_UM 12`, 입력·해제 강제 틈). 2026-08-17 에 그 바닥이
      **저주파(1/f)** 라는 데까지는 갔는데 (조리개·조명 무관, 누적은 N^-0.2), 출처가
      **기준전압·전원(공통)** 인지 **센서 자체(독립)** 인지는 안 갈랐다.

        공통 -> 디커플링·기준 개선으로 잡힌다
        독립 -> 센서 한계, 이 방향은 닫힌다

      `keys noise` 에 셀 간 상관을 하나 더하면 된다.
